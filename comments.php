    ...
    ?>">
        <div id="<?php $comments->theId(); ?>">
            <div class="comment-author">
                <?php $comments->gravatar('128'); ?> <!-- 显示评论者的Gravatar头像 -->
                <b><?php CommentAuthor($comments); ?></b> <!-- 显示评论者的名字 -->
                <?php if ($comments->authorId == $comments->ownerId) { ?>
                    <span class="author-icon">Author</span> <!-- 如果是作者，显示作者图标 -->
                <?php } ?>
                <?php if ($comments->status == 'waiting') { ?>
                    <em>您的评论正等待审核！</em> <!-- 如果评论在等待审核，显示提示信息 -->
                <?php } ?>
            </div>
            <div class="comment-meta">
                <time><?php $comments->date(); ?></time> <!-- 显示评论日期 -->
            </div>
            <div class="comment-content heti">
                <?php $comments->content(); ?> <!-- 显示评论内容 -->
            </div>
        ...
        <?php if ($comments->children) { ?>
            <div class="comment-children">
                <?php $comments->threadedComments($options); ?> <!-- 递归显示子评论 -->
            </div>
        <?php } ?>
    </li>
<?php } ?>
            ...
            <form method="post" action="<?php $this->commentUrl() ?>"
                ...
                <p <?php if (!$this->user->hasLogin()): ?>class="textarea" <?php endif; ?>>
					<textarea name="text" id="textarea"
                              placeholder="欢迎参与讨论~ 评论支持部分 Markdown 语法，有回复时会通过邮件通知~"
